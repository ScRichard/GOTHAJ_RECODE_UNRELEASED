...
import vip.gothaj.client.event.events.EventBlockBreak;
import vip.gothaj.client.event.events.EventBlockDamage;
...
public class PlayerControllerMP
{
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face)
    {
        ...
        {
        	EventBlockBreak event = new EventBlockBreak(loc);
            ...
            else if (!this.isHittingBlock || !this.isHittingPosition(loc))
            {
                ...
                EventBlockDamage event1 = new EventBlockDamage(block1.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, loc), loc);
            	Client.INSTANCE.getEventBus().call(event1);

                if (flag && event1.getPlayerRelativeBlockHardness() >= 1.0F)
                {
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing)
    {
        ...
        else if (this.isHittingPosition(posBlock))
        {
            ...
            else
            {
            	...
            	EventBlockDamage event = new EventBlockDamage(block.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, posBlock), posBlock);
            	Client.INSTANCE.getEventBus().call(event);
            	
                this.curBlockDamageMP += event.getPlayerRelativeBlockHardness();
    ...
    public void attackEntityEvent(EntityPlayer playerIn, Entity targetEntity)
    {
        ...
        if(event.isForced()) {
            ...
            mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C02PacketUseEntity(targetEntity, C02PacketUseEntity.Action.ATTACK));
        }else {
			this.syncCurrentPlayItem();
			this.netClientHandler
					.addToSendQueue(new C02PacketUseEntity(targetEntity, C02PacketUseEntity.Action.ATTACK));
        }
        if (this.currentGameType != WorldSettings.GameType.SPECTATOR)
        {
