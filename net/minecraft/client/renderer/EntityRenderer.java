...
import vip.gothaj.client.event.events.EventReach;
import vip.gothaj.client.event.events.EventRender3D;
import vip.gothaj.client.event.events.EventRenderGui;
import vip.gothaj.client.event.events.EventRotation;
import vip.gothaj.client.event.events.EventSkyColor;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            EventReach event = new EventReach(3);
            Client.INSTANCE.getEventBus().call(event);
            
            int i = (int) event.getRange();
            ...
            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > i)
            {
                this.pointedEntity = null;
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        ...
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase && !Client.INSTANCE.getClientSettings().hurtcam.isEnabled())
        {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.theWorld != null)
            {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null)
                {
                    ...
                    Client.INSTANCE.getOutline().execute(ShaderType.Render2D, partialTicks);
                    
                    EventRenderGui event = new EventRenderGui(new ScaledResolution(mc));
                    Client.INSTANCE.getEventBus().call(event);
            ...
            Client.INSTANCE.getBloom().execute(ShaderType.Render2D, partialTicks);
        }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        EventRender3D event = new EventRender3D(mc.timer.renderPartialTicks);
        Client.INSTANCE.getEventBus().call(event);
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        EventSkyColor event = new EventSkyColor((float)vec31.xCoord, (float)vec31.yCoord, (float)vec31.zCoord);
        Client.INSTANCE.getEventBus().call(event);
        vec31.xCoord = event.getRed() / 255;
        vec31.yCoord = event.getGreen() / 255;
        vec31.zCoord = event.getBlue() / 255;
