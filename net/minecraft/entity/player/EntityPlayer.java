...
import vip.gothaj.client.event.events.EventKnockBack;
    ...
    public void attackTargetEntityWithCurrentItem(final Entity targetEntity) {
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
            ...
            if (f > 0.0f || f2 > 0.0f) {
                ...
                if (flag3) {
                	EventKnockBack eventAttackSlowdown = new EventKnockBack(0.6, false, 1, 0, false, false);
                    Client.INSTANCE.getEventBus().call(eventAttackSlowdown);
                    
                    float yaw = this.rotationYaw;
                    
                    if (i > 0) {
                        if(!eventAttackSlowdown.isCancelled()) {
                        	for(int power = 0; power < eventAttackSlowdown.getPower(); power++) {
                        		targetEntity.addVelocity((double)(-MathHelper.sin(yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), eventAttackSlowdown.isReduceY() ? 0 : 0.1, (double)(MathHelper.cos(yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                        	}
                        	this.motionX *= eventAttackSlowdown.getMotion();
                        	this.motionZ *= eventAttackSlowdown.getMotion();
                        	this.setSprinting(false);
                        }
                    }
                    else if (eventAttackSlowdown.isFull() && Minecraft.getMinecraft().thePlayer.hurtTime > 0) {
                    	for(int power = 0; power < eventAttackSlowdown.getPostPower(); power++) {
                    		targetEntity.addVelocity((double)(-MathHelper.sin(yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), eventAttackSlowdown.isReduceY() ? 0 : 0.1, (double)(MathHelper.cos(yaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                    	}
                    	if(!eventAttackSlowdown.isCancelled()) {
                    		this.motionX *= eventAttackSlowdown.getMotion();
