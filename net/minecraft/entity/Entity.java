...
import vip.gothaj.client.event.events.EventGround;
import vip.gothaj.client.event.events.EventMoveFlying;
import vip.gothaj.client.event.events.EventSafeWalk;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            EventSafeWalk event1 = new EventSafeWalk(flag);
            ...
            EventGround event = new EventGround(this.isCollidedVertically && d4 < 0.0D);
    ...
    {
        EventMoveFlying event = new EventMoveFlying(strafe, forward, friction, this.rotationYaw);
        Client.INSTANCE.getEventBus().call(event);
        ...
        if (f >= 1.0E-4F)
        {
            ...
            strafe = strafe * f;
            forward = forward * f;
            float f1 = MathHelper.sin(finalYaw * (float)Math.PI / 180.0F);
            float f2 = MathHelper.cos(finalYaw * (float)Math.PI / 180.0F);
    ...
    {
    	Vec3 vec3 = this.getPositionEyes(partialTicks);
        Vec3 vec31 = this.getLookCustom(partialTicks, yaw, pitch);
        Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, false, true);
    }
    public Vec3 getLookCustom(float partialTicks, float yaw, float pitch)
    {
